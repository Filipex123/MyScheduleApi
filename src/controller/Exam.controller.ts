...
import { interfaces, controller, request, response, httpPost, httpGet, httpDelete } from 'inversify-express-utils';
...
import { IDeleteExamUseCase } from '../usecase/interface/DeleteExam.usecase.interface';
...
export default class ExamController implements interfaces.Controller {
  constructor(
    ...
    @inject(InjectionEnum.DeleteExamUseCase) private readonly deleteExamUsecase: IDeleteExamUseCase,
  ...
  @httpDelete('/:id')
  public async deleteExam(@request() req: express.Request, @response() res: express.Response) {
    try {
